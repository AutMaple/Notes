...
2. 服务器每次结束一个事件循环之前，它都会调用 flushAppendOnlyFile 函数，考虑是否需要将 aof_buf 缓冲区中的内容写入和保存到AOF文件里面。flushAppendOnlyFile 函数的行为由服务器配置的 appendfsync 选项的值来决定：always,everysec,no。always 表示只要 aof_buf 缓冲区有内容就进行写入，everysec 表示每秒进行一次刷盘，no 表示刷盘的时机由操作系统控制。如果用户没有主动为 appendfsync 选项设置值，那么 appendfsync 选项的默认值为 everysec。
